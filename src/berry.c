...
// checksum
u32 sub_80B48F8(u8 *enigmaBerry) {
	u8 *description1;
	u8 *description2;
	unsigned int i;
	u32 checksum;
...
void FieldObjectInteractionGetBerryTreeData(void) {
	...
	if ((u8)sub_8060234(unk, num, group)) {
...
void FieldObjectInteractionPickBerryTree(void) {
	...
	id = FieldObjectGetBerryTreeId(gSelectedMapObject);
	berry = GetBerryTypeByBerryTreeId(id);
	gUnknown_0202E8CC = (u8)sub_80A9424(BerryTypeToItemId(berry), GetBerryCountByBerryTreeId(id));
}
